...
# U = 100.
# 从0到100进行切分
mues = np.linspace(0.05, 5, 500)
rhoes = np.linspace(1, 50, 500)
Use = np.linspace(1, 20, 500)
datasets = []
for mu, rho, U in tqdm(zip(mues, rhoes, Use), total=len(mues)):
    sum_res_list = []
    print(f"正在处理 mu={mu:.2f}, rho={rho:.2f}, U={U:.2f}...")
    # %%
    # --- 核心变量定义 (与专业范例一致) ---
    Vc = mesh.cellVolumes
    ...
    airfoilsFace = mesh.physicalFaces["obstacle"]  # <-- 此处为唯一修改
...
np.savez(results_dir, datasets=datasets)
